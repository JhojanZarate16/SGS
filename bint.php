...
if(isset($_REQUEST["search"]) && $_REQUEST["search"] != "")
{
	...
	$pagination->rowCount("SELECT * FROM internacional WHERE guia LIKE '%".$search."%'");
	$pagination->config(3, 4);
	$sql = "SELECT * FROM internacional WHERE guia LIKE '%".$search."%' ORDER BY id_inter ASC LIMIT $pagination->start_row, $pagination->max_rows";
...
{
	$pagination->rowCount("SELECT * FROM internacional");
	$pagination->config(3, 4);
	$sql = "SELECT * FROM internacional ORDER BY id_inter ASC LIMIT $pagination->start_row, $pagination->max_rows";
	$query = $connection->prepare($sql);
	$query->execute();
	$model = array();
while($rows = $query->fetch())
...
<head>
<meta charset="UTF-8">
	<title>Buscador De Mnesajeria Internacional</title>
...
<div id="contenedor">
	...
	<div align="center" class="b">
		<table border="0" id="tit">
			<tr>
				<td colspan="2" align="center"><div class="logo">Buscador Mensajeria<br>Internacional</div></td>
			</tr>
			<tr>
		...
		?>
		        <tr align="center">
					...
					<td><a href="javascript:prueba('morint.php?per=<?php echo $row["id_inter"]?>')" tabindex="1"><img src="img/plus.png" width="20" height="20"></a></td>
				</tr>
		<?php	
		    }
		?>
