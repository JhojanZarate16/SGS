...
if(isset($_REQUEST["search"]) && $_REQUEST["search"] != "")
{
	...
	$sql = "SELECT * FROM correspondencia WHERE guia LIKE '%".$search."%' ORDER BY id_user ASC LIMIT $pagination->start_row, $pagination->max_rows";
...
else
{
	...
	$sql = "SELECT * FROM correspondencia ORDER BY id_correspondencia ASC LIMIT $pagination->start_row, $pagination->max_rows";
	$query = $connection->prepare($sql);
	$query->execute();
	$model = array();
while($rows = $query->fetch())
	...
	<div align="center" class="b">
		...
		?>
		        <tr align="center">
					...
					<td><a href="javascript:prueba('morcor.php?per=<?php echo $row["id_correspondencia"]?>')" tabindex="1"><img src="img/plus.png" width="20" height="20"></a></td>
				</tr>
		<?php	
		    }
		?>
